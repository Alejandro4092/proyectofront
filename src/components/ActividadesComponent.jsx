...
export class ActividadesComponent extends Component {
	...
	loadActividadesInscritas = async () => {
		...
		try {
			...
			const idsInscritas = actividadesDelEvento.map(
				(act) => act.idEventoActividad,
			);
			this.setState({
		...
		if (result.isConfirmed) {
			try {
				...
				const todasInscripciones =
					await serviceInscripciones.obtenerInscripciones();
				const inscripcion = todasInscripciones.find(
					(ins) =>
						ins.idEventoActividad === idEventoActividad &&
				...
				let token = this.context.token;
				await serviceInscripciones.desinscribirse(
				...
				Swal.fire({
					...
					text:
						error.response?.data ||
						"Error al desinscribirse. Por favor, intenta de nuevo.",
				});
			}
		}
	};
				...
				{this.state.mostrarModal && (
					<div className="actividades-modal-overlay" onClick={this.cerrarModal}>
						...
						>
							...
							<h2>
								Inscribirse en{" "}
								{this.state.actividadSeleccionada?.nombreActividad}
							</h2>
								...
								<div className="actividades-form-group">
									<label className="custom-switch">
										<input
											...
											onChange={(e) =>
												this.setState({ esCapitan: e.target.checked })
											}
										/>
