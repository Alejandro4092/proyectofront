...
export class ListaEquiposComponent extends Component {
	...
	getEventoActividad = async () => {
		try {
			return await serviceActividades.getEventoActividad(
				this.props.idEvento,
	...
	guardarResultado = async (e) => {
		...
		const { idEquipoLocal, idEquipoVisitante, puntosLocal, puntosVisitante } =
			this.state.formularioResultado;

		// Validaciones
		if (!idEquipoLocal || !idEquipoVisitante) {
			this.setState({
				mensajeErrorResultado: "Debes seleccionar ambos equipos",
			});
		...
		if (idEquipoLocal === idEquipoVisitante) {
			this.setState({
				mensajeErrorResultado: "Los equipos deben ser diferentes",
			});
			setTimeout(() => this.setState({ mensajeErrorResultado: "" }), 1500);
			return;
		}
		...
		try {
			...
			await servicePartidos.createPartidoResultado(
				partidoResultado,
				this.context.token,
			);
		...
		return (
			<div className="equipos-container">
				...
				<div className="equipos-header">
					<h1>Equipos</h1>
					{this.state.capitan != null && this.state.capitan != "" && (
						<h1 className="capitan-info">
							Capitán:{" "}
							<span className="capitan-nombre">
								{this.state.capitan.usuario}
							</span>
						</h1>
					)}
				...
				{this.state.mostrarFormularioResultado && (
					<div
						className="equipos-modal-overlay"
						onClick={this.cerrarFormularioResultado}
					>
						<div
							className="equipos-modal-content"
							onClick={(e) => e.stopPropagation()}
						>
							<button
								className="equipos-modal-close"
								onClick={this.cerrarFormularioResultado}
							>
								&times;
							</button>
											...
											{this.state.equipos
												.filter(
													(equipo) =>
														equipo.idEquipo !==
														parseInt(
															this.state.formularioResultado.idEquipoVisitante,
														),
												)
												.map((equipo) => (
											...
											{this.state.equipos
												.filter(
													(equipo) =>
														equipo.idEquipo !==
														parseInt(
															this.state.formularioResultado.idEquipoLocal,
														),
												)
												.map((equipo) => (
													<option key={equipo.idEquipo} value={equipo.idEquipo}>
