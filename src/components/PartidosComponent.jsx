...
export class PartidosComponent extends Component {
	...
	aplicarFiltroActividad = async (idEvento, idActividad) => {
		...
		try {
			...
			servicePartidos
				.getPartidosPorActividad(idEventoActividad)
				.then((data) => {
	...
	handleSubmitResultado = async (e) => {
		...
		// Validaciones
		if (
			!formulario.idEventoActividad ||
			!formulario.idEquipoLocal ||
			!formulario.idEquipoVisitante
		) {
			this.setState({
		...
		if (formulario.idEquipoLocal === formulario.idEquipoVisitante) {
			this.setState({
				mensajeError: "El equipo local y visitante deben ser diferentes",
			});
			return;
		}
		...
		try {
			...
			this.setState({
				mensajeError:
					...
					"Error al crear el resultado del partido",
				guardandoResultado: false,
			});
		}
	};
	...
	render() {
		...
		return (
			<div className="partidos-container">
				<h1>
					<FaFutbol /> Resultados de Partidos
				</h1>

				{/* Botón para mostrar/ocultar formulario */}
				{this.context.user &&
					(this.context.user.role === "CAPITAN" ||
											...
											if (idEvento && idActividad) {
												try {
													const idEventoActividad =
														await serviceActividades.getEventoActividad(
															parseInt(idEvento),
															parseInt(idActividad),
														);
													this.setState((prevState) => ({
						...
						return (
							...
							>
								...
								<div className="partido-content">
									...
									>
										<div
											className={`equipo local ${ganador === "local" ? "ganador" : ""}`}
										>
											<div className="nombre-equipo">
												<FaUsers />{" "}
												{equipoLocal.nombreEquipo ||
													`Equipo ${partido.idEquipoLocal}`}
											</div>
											<div className="puntos-equipo">{partido.puntosLocal}</div>
										</div>
									</NavLink>
									...
									>
										<div
											className={`equipo visitante ${ganador === "visitante" ? "ganador" : ""}`}
										>
											<div className="nombre-equipo">
												<FaUsers />{" "}
												{equipoVisitante.nombreEquipo ||
													`Equipo ${partido.idEquipoVisitante}`}
											</div>
