...
export class PagosComponent extends Component {
    ...
    procesarPago = async (e) => {
        e.preventDefault();
        const { pagoSeleccionado, cantidadPago, cursoSeleccionado } = this.state;
                            ...
                            <table className="pagos-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Estado</th>
                                        {pestanaActiva === 'pendientes' && <th>Acción</th>}
                                    </tr>
                                </thead>
                                <tbody>
                                    {pagosFiltrados.map((pago) => (
                                        <tr key={pago.id}>
                                            ...
                                            <td>
                                                <span className={`estado-badge ${this.getEstadoClase(pago.estado)}`}>
                                            ...
                                            {pestanaActiva === 'pendientes' && (
                                                <td>
                                                    {pago.estado?.toLowerCase() !== 'pagado' && (
                                                        <button 
                                                            className="btn-pagar"
                                                            onClick={() => this.abrirModalPago(pago)}
                                                        >
                                                            Pagar
                                                        </button>
                                                    )}
                ...
                {this.state.mostrarModalPago && (
                    <div className="modal-overlay" onClick={this.cerrarModalPago}>
                        <div className="modal-content-pago" onClick={(e) => e.stopPropagation()}>
                            <button className="modal-close" onClick={this.cerrarModalPago}>&times;</button>
                            <h2>Registrar Pago</h2>
                            
                            <div className="pago-info">
                                <p><strong>Actividad:</strong> {this.state.pagoSeleccionado?.actividad}</p>
                                <p><strong>Precio Total:</strong> {this.state.pagoSeleccionado?.precioTotal}€</p>
                                <p><strong>Cantidad Pagada:</strong> {this.state.pagoSeleccionado?.cantidadPagada}€</p>
                                <p><strong>Pendiente:</strong> {(this.state.pagoSeleccionado?.precioTotal - this.state.pagoSeleccionado?.cantidadPagada) || 0}€</p>
                            </div>
                            ...
                            <form onSubmit={this.procesarPago}>
                                ...
                                <div className="modal-actions-pago">
                                    <button type="button" className="btn-cancel-pago" onClick={this.cerrarModalPago}>
                                        Cancelar
                                    </button>
