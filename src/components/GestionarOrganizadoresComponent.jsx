...
class GestionarOrganizadoresComponent extends Component {
	...
	handleSubmit = async (e) => {
		...
		try {
			...
			await serviceOrganizadores.createOrganizador(
				idUsuarioSeleccionado,
				token,
			);
			...
			Swal.fire({
				title: "Error",
				text:
					error.response?.data?.message || "No se pudo crear el organizador",
				icon: "error",
				confirmButtonText: "Aceptar",
			});
		} finally {
	...
	handleEliminar = async (organizador) => {
		...
		if (result.isConfirmed) {
			try {
				await serviceOrganizadores.deleteOrganizador(
				...
				Swal.fire({
					...
					text:
						...
						"No se pudo eliminar el organizador",
					icon: "error",
					confirmButtonText: "Aceptar",
				});
			}
	...
	render() {
		...
		const usuariosDisponibles = todosUsuarios.filter(
			(usuario) => !idsOrganizadores.includes(usuario.idUsuario),
		);
				...
				<div className="content">
					...
					<div className="crear-organizador-section">
						...
						<form
							...
							className="crear-organizador-form"
						>
							...
							<button
								type="submit"
								className="organizadores-btn-submit"
									...
									<tbody>
										{organizadores.map((org) => (
											<tr key={org.idUsuario}>
												...
												<td>
													<button
														className="organizadores-btn-eliminar"
