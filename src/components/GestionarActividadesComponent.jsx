...
export class GestionarActividadesComponent extends Component {
	...
	asociarActividad = async (e) => {
		...
		try {
			await serviceActividades.asociarEventoActividad(
				idEvento,
				idActividad,
				token,
			);
	...
	eliminarActividad = async (idEventoActividad, nombreActividad) => {
		...
		if (result.isConfirmed) {
			...
			try {
				await serviceActividades.eliminarEventoActividad(
					idEventoActividad,
					token,
				);
	...
	eliminarActividadPanic = async (idEventoActividad, nombreActividad) => {
		const result = await Swal.fire({
			title: "⚠️ ¡ADVERTENCIA!",
			html: `<p>Esta acción eliminará <strong>COMPLETAMENTE</strong> la actividad "${nombreActividad}".</p>
				   <p style="color: #d33; font-weight: bold;">Esta acción no se puede deshacer.</p>`,
			icon: "warning",
			showCancelButton: true,
			confirmButtonColor: "#d33",
			cancelButtonColor: "#3085d6",
		...
		if (result.isConfirmed) {
			...
			try {
				await serviceActividades.eliminarEventoActividadPanic(
					idEventoActividad,
					token,
				);
				...
				Swal.fire({
					icon: "success",
					title: "¡Eliminada completamente!",
					text: "La actividad se ha eliminado por completo",
					timer: 2000,
					showConfirmButton: false,
				});
			...
			<div className="gestionar-actividades-container">
				<div className="gestionar-actividades-wrapper">
					<div className="gestionar-actividades-header">
						<h1>
							<FaTasks /> Gestionar Actividades del Evento
						</h1>
						<Link
							to={`/actividades/${this.props.idEvento}`}
							className="ga-btn-volver"
						>
							<FaArrowLeft /> Volver
						</Link>
					</div>
					...
					<div className="gestionar-actividades-content">
						...
						<div className="actividades-asociadas-section">
							...
							{this.state.actividadesAsociadas.length === 0 ? (
								<p className="no-actividades">
									No hay actividades asociadas a este evento
								</p>
							) : (
									...
									{this.state.actividadesAsociadas.map((actividad) => (
										...
										>
											...
											<div className="actividad-buttons">
												...
												<button
													className="ga-btn-eliminar-todo"
													onClick={() =>
														this.eliminarActividadPanic(
															actividad.idEventoActividad,
															actividad.nombreActividad,
														)
													}
												>
													<FaExclamationTriangle /> Eliminar Todo
												</button>
											</div>
										</div>
									))}
